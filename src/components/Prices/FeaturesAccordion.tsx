  ...
  return (
    <div className=" mx-32 mt-4">
      <div className="border-y transition-all">
        <div
          className={`transition-colors flex items-center justify-between p-4 cursor-pointer ${theme === 'dark' ? 'hover:bg-gray-800' : 'hover:bg-gray-100'} ${openIndex ? theme === 'dark' ? 'bg-gray-800' : 'bg-gray-100' : ''}`}
          ...
          <div className="p-4">
            <div className="flex gap-4 p-4 pt-0">
              <button
                ...
                className="w-max text-nowrap select-none font-source-serif text-lg bg-cyan-400 px-4 py-2 rounded-xl shadow-md text-black [&:not(:disabled):hover]:shadow-lg [&:not(:disabled):hover]:bg-cyan-300 [&:not(:disabled):active]:shadow-sm [&:not(:disabled):active]:bg-cyan-500 [&:not(:disabled):active]:scale-90 transition-all disabled:cursor-not-allowed disabled:grayscale"
            ...
            <div className="px-4 grid grid-cols-2 gap-4 relative">
              {
                details.map((detail, index) => (
                  <label
                    ...
                    className={`transition-all select-none ${openIndex ? 'pointer-events-auto' : 'pointer-events-none'} bg-gray-200 [&:has(input:checked)]:bg-cyan-400 py-4 px-8 cursor-pointer [&:has(input:focus-visible:not(checked))]:bg-gray-300 [&:has(input:focus-visible:checked)]:bg-cyan-500 [&:has(input:focus-visible)]:scale-[1.02] hover:bg-gray-300 [&:hover:has(input:checked)]:bg-cyan-500 hover:scale-[1.02] active:scale-[0.98]`}
